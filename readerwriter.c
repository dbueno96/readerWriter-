...
#define SEMNAME11 "/mysemaphoredd1"
#define SEMNAME22 "/mysemaphore2dd"
...
int main() {
	...
	roomEmpty = sem_open(SEMNAME11, O_CREAT, S_IRUSR | S_IWUSR, 1);
	s2 = sem_open(SEMNAME22, O_CREAT, S_IRUSR | S_IWUSR, 1);
...
void  INThandler(int sig)
{
     ...
     c = getchar();
     if (c == 'y' || c == 'Y') {
 		sem_close(roomEmpty); sem_unlink("semReaders");
 		sem_close(s2); sem_unlink("semWriters");
		exit(0);
     } else
          	signal(SIGINT, INThandler);
